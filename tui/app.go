...
func (a *App) Init() tea.Cmd {
	log.Println("App.Init() called")
	// Return the initial command from the first view
	if a.initCmd != nil {
		log.Println("App.Init() returning stored initCmd")
		cmd := a.initCmd
		a.initCmd = nil // Clear it so it doesn't run again
		return cmd
	}
	log.Println("App.Init() calling currentView.Init()")
	return a.currentView.Init()
}

func (a *App) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	// Log all messages for debugging
	log.Printf("App.Update received message of type: %T", msg)
